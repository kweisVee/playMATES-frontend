  ...
  const renderCardSkeletons = (count: number, gridClasses: string) => (
    <div className={gridClasses}>
      {Array.from({ length: count }).map((_, index) => (
        <Card key={index} className="rounded-2xl border-slate-200/80 bg-white p-0 shadow-[0_16px_56px_-32px_rgba(15,23,42,0.35)]">
      ...
      <div className="relative min-h-screen overflow-hidden bg-gradient-to-b from-emerald-50 via-white to-teal-50/60">
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            <Card className="rounded-2xl border-emerald-200/80 bg-gradient-to-br from-emerald-50 to-white p-6 shadow-[0_16px_56px_-32px_rgba(6,95,70,0.45)]">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-muted-foreground mb-1">Meetups Joined</p>
                  <p className="text-3xl font-bold text-slate-900">{stats.totalJoined}</p>
                </div>
            ...
            <Card className="rounded-2xl border-teal-200/80 bg-gradient-to-br from-teal-50 to-white p-6 shadow-[0_16px_56px_-32px_rgba(13,148,136,0.45)]">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-muted-foreground mb-1">Meetups Hosted</p>
                  <p className="text-3xl font-bold text-slate-900">{stats.totalHosted}</p>
                </div>
            ...
            <Card className="rounded-2xl border-slate-200/80 bg-gradient-to-br from-slate-50 to-white p-6 shadow-[0_16px_56px_-32px_rgba(15,23,42,0.35)]">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-muted-foreground mb-1">Upcoming</p>
                  <p className="text-3xl font-bold text-slate-900">{stats.upcomingCount}</p>
                </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Link href="/create-meetup">
              <Card className="rounded-2xl border-emerald-200/80 bg-gradient-to-br from-emerald-50 to-white p-6 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_20px_60px_-30px_rgba(6,95,70,0.55)] cursor-pointer">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-emerald-100 rounded-full flex items-center justify-center">
                    <Plus className="h-6 w-6 text-emerald-700" />
                  </div>
                  <div>
                    <h3 className="font-semibold text-lg text-slate-900">Create Meetup</h3>
            ...
            <Link href="/browse-all-meetups">
              <Card className="rounded-2xl border-teal-200/80 bg-gradient-to-br from-teal-50 to-white p-6 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_20px_60px_-30px_rgba(13,148,136,0.55)] cursor-pointer">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-teal-100 rounded-full flex items-center justify-center">
                    <Search className="h-6 w-6 text-teal-700" />
                  </div>
                  <div>
                    <h3 className="font-semibold text-lg text-slate-900">Browse Meetups</h3>
            ...
            <Link href="/my-meetups">
              <Card className="rounded-2xl border-slate-200/80 bg-gradient-to-br from-slate-50 to-white p-6 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_20px_60px_-30px_rgba(15,23,42,0.45)] cursor-pointer">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-slate-100 rounded-full flex items-center justify-center">
                    <List className="h-6 w-6 text-slate-700" />
                  </div>
                  <div>
                    <h3 className="font-semibold text-lg text-slate-900">My Meetups</h3>
        ...
        >
          <div className="mb-4 flex items-start justify-between gap-4">
            <div>
              <h2 className="text-2xl font-bold text-slate-900">Your Upcoming Meetups</h2>
          ...
          <div className="rounded-3xl border border-emerald-100/80 bg-white/75 p-4 shadow-[0_22px_70px_-38px_rgba(15,23,42,0.45)] backdrop-blur-sm md:p-6">
            {loading ? (
              ...
              <Card className="rounded-2xl border-slate-200/80 p-12 text-center shadow-[0_16px_56px_-32px_rgba(15,23,42,0.35)]">
                ...
                <h3 className="text-xl font-semibold text-slate-900 mb-2">No upcoming meetups</h3>
